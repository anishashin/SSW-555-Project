...
def us05us06MarriageDivorceBeforeDeath(id_name):
        '''Returns an error message if a couple's marriage or divorce occurs after one or both of their death's'''
        ...
        husband_id = FAM_IDS[id_name][2]
        husband_death = INDI_IDS[husband_id][5]
        wife_id = FAM_IDS[id_name][4]
        wife_death = INDI_IDS[wife_id][5]
            
        if husband_death != 'NA' and wife_death != 'NA':           
            husband_death = convertToDate(husband_death)
            wife_death = convertToDate(wife_death)
        elif divorce == 'NA':
            if husband_death != 'NA':
                husband_death = convertToDate(husband_death)
                if husband_death > marriage_date:
                    return "ERROR: FAMILY: US05: " + id_name + ". The husband has died before they were married on " + str(marriage) + "."
                if wife_death > marriage_date:
                    return "ERROR: FAMILY: US05: " + id_name + ". The wife has died before they were divorced on " + str(marriage) + "."
        elif divorce != 'NA':
            divorce_date = convertToDate(divorce)
            if husband_death != 'NA':
                if husband_death > marriage_date:
                    return "ERROR: FAMILY: US05: " + id_name + ". The husband has died before they were married on " + str(marriage) + "."
                elif husband_death > divorce_date:
                    return "ERROR: FAMILY: US05: " + id_name + ". The husband has died before they were married on " + str(marriage) + "."
            else:
                if wife_death > marriage_date:
                    return "ERROR: FAMILY: US05: " + id_name + ". The wife has died before they were divorced on " + str(marriage) + "."
                elif wife_death > divorce_date:
                    return "ERROR: FAMILY: US05: " + id_name + ". The wife has died before they were divorced on " + str(marriage) + "."
            
        '''if check_husb_m < 0:
           return "ERROR: FAMILY: US05: " + id_name + ". The husband has died before they were married on " + str(marriage) + "."
        elif check_wife_m < 0:
            return "ERROR: FAMILY: US05: " + id_name + ". The wife has died before they were married on " + str(marriage) + "."
        elif check_husb_d < 0:
            return "ERROR: FAMILY: US06: " + id_name + ". The husband died before they were divorced on " + str(divorce) + "."
        elif check_wife_d < 0:
            return "ERROR: FAMILY: US06: " + id_name + ". The wife died before they were divorced on " + str(divorce) + "."'''
            ...
            if FAM_IDS[id_name][2] != 'NA' and FAM_IDS[id_name][4] != 'NA':
                if us02BirthBeforeMarriage(marr, id_name) != []:
                    for error_message in us02BirthBeforeMarriage(marr, id_name):
                        print(error_message)
                if us05us06MarriageDivorceBeforeDeath(id_name) != None:
                    print(us05us06MarriageDivorceBeforeDeath(id_name))
