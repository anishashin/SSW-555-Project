...
def us23UniqueNameAndBirthDate(id_name):
    ...
    for id in INDI_IDS.keys():
        name = INDI_IDS[id][0]
        birthday = INDI_IDS[id][2]
        birth_date = convertToDate(birthday)
        if [name, birth_date] not in people:
            people.append([name, birth_date])
        else:
            if id == id_name:
                error_messages += ['ERROR: INDIVIDUAL: US23: ' + id_name + ': There is more than one person with the same name ' + name + ' and the same birthday ' + str(birth_date)]
    return error_messages
...
def us29ListDeceased():
    '''Lists all the deceased in the file'''
    deceased = []
    error_messages = []
    error_messages.append('US29: The Following Individuals are Deceased:')
    for id in INDI_IDS:
        if INDI_IDS[id][4] == 'False':
            if INDI_IDS[id][5] != 'NA':
                name = INDI_IDS[id][0]
                death = INDI_IDS[id][5]
                death_date = convertToDate(death)
                if [name, death_date] not in deceased:
...
def main():
    ...
    if us29ListDeceased != []:
        for person in us29ListDeceased():
            print(person)
        print(" ")
    ...
    for id_name in INDI_IDS:
        if INDI_IDS[id_name][2] != 'NA':
            ...
            if us23UniqueNameAndBirthDate(id_name) != []:
                for error_message in us23UniqueNameAndBirthDate(id_name):
                    print(error_message)
